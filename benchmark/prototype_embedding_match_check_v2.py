...
Second-model rerun of `prototype_embedding_match_check.py`. Same methodology, same 60
messages, same 4 lore entries — only the embedding model changes, so the two runs' output
is directly comparable.

Uses LM Studio's OpenAI-compatible `/v1/embeddings` endpoint with
`text-embedding-bge-small-en-v1.5` (BAAI's BGE-small, plain BERT architecture, contrastively
trained — a different base architecture and training approach than nomic-embed-text-v1.5's
Nomic-BERT, chosen deliberately for that difference). 384-dim, Q8_0 GGUF, 36.81 MB on disk,
downloaded for this test via `lms get` from CompendiumLabs/bge-small-en-v1.5-gguf on Hugging
Face — confirmed as `type: embeddings` by `GET /api/v0/models` on this machine, not assumed.
...
EMBEDDING_MODEL = "text-embedding-bge-small-en-v1.5"
