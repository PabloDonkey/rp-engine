...
async def test_json_session_store_saves_and_loads_session(tmp_path: Path) -> None:
    ...
    session = Session.create(user_id=user_id, character_id="belzebuth", world_id="default")
...
async def test_json_session_store_tracks_active_session_per_user(tmp_path: Path) -> None:
    ...
    session = Session.create(user_id=user_id, character_id="alice", world_id="default")

    await store.save(session)
    await store.set_active_for_user(user_id=user_id, session_id=session.id)
    active = await store.get_active_for_user(user_id=user_id)
